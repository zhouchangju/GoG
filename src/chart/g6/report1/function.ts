...
const processNodesEdges = (
  ...
  nodes.forEach(node => {
    ...
    if (cachePosition) {
      ...
      if (config.manipulatePosition && !node.x && !node.y) {
        node.x =
          config.manipulatePosition.x +
          30 * Math.cos(Math.random() * Math.PI * 2);
        node.y =
          config.manipulatePosition.y +
          30 * Math.sin(Math.random() * Math.PI * 2);
      }
    }
  });
  ...
  edges.forEach(edge => {
    ...
    if (!sourceNode.x && !sourceNode.y && config.manipulatePosition) {
      sourceNode.x =
        config.manipulatePosition.x +
        30 * Math.cos(Math.random() * Math.PI * 2);
      sourceNode.y =
    ...
    if (!targetNode.x && !targetNode.y && config.manipulatePosition) {
      targetNode.x =
        config.manipulatePosition.x +
        30 * Math.cos(Math.random() * Math.PI * 2);
      targetNode.y =
  ...
  descreteNodes.forEach(node => {
    if (!node.x && !node.y) {
      node.x =
        config.descreteNodeCenter.x +
        30 * Math.cos(Math.random() * Math.PI * 2);
      node.y =
        config.descreteNodeCenter.y +
        30 * Math.sin(Math.random() * Math.PI * 2);
    }
  });
...
const getForceLayoutConfig = (graph, largeGraphMode, configSettings) => {
  ...
  console.error(edgeStrength, linkDistance);
...
const getNeighborMixedGraph = (
  ...
  config.manipulatePosition = { x: centerNodeModel.x, y: centerNodeModel.y };
...
const manageExpandCollapseArray = (
  ...
  config.manipulatePosition = { x: model.x, y: model.y };
